...
function ModuleHandler:include(ModuleName)
    if not GlobalSYC.Modules then
        warn("Modules table is missing!")
        return
    end
    if type(ModuleName) ~= "string" then
        warn("Module name must be a string.")
        return
    end
...
do
    function SYC.Modules.Connect(onething, secondthing)
        if not onething or not secondthing then
            warn("Invalid connection arguments.")
            return
        end
...
do
    ...
    setmetatable(DrawingClass, {
        __call = function(_, args)
            if not Drawing or type(args) ~= "table" or not args[1] then
                warn("Invalid Drawing.new call. Ensure your environment supports `Drawing.new`.")
                return
            end
            local newObject = Drawing.new(args[1])
            for property, value in pairs(args[2] or {}) do
...
do
    ...
    function Loops:AddToHeartbeat(Name, Function)
        if not Function or type(Function) ~= "function" then
            warn("Invalid function for Heartbeat.")
            return
        end
    ...
    function Loops:AddToRenderStepped(Name, Function)
        if not Function or type(Function) ~= "function" then
            warn("Invalid function for RenderStepped.")
            return
        end
...
do
    function SYC.Modules.PerlinNoise(offset, speed, time)
        if type(offset) ~= "number" or type(speed) ~= "number" or type(time) ~= "number" then
            warn("Invalid Perlin noise arguments.")
            return 0
        end
...
do
    function SYC.Modules.UI.RichTextToNormalText(str)
        if type(str) ~= "string" then
            warn("RichText input must be a string.")
            return ""
        end
        local output_string = str
        while true do
            if not output_string:find("<") and not output_string:find(">") then
                break
            end
...
do
    function SYC.Modules.UI:GetTextBoundary(Text, Font, Size, Resolution)
        if type(Text) ~= "string" or not Font or type(Size) ~= "number" then
            warn("Invalid TextBoundary arguments.")
            return 0, 0
        end
        local Bounds = textService:GetTextSize(Text, Size, Font, Resolution or Vector2New(1920, 1080))
        return Bounds.X or 0, Bounds.Y or 0
    end
end
